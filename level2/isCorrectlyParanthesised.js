/**
 * Detects whether a given string is correctly paranthesised,
 * only considering parantheses, i.e. '(' and ')'.
 * Returns true or false.
 */


function isCorrectlyParanthesised(string) {
  let counter = 0
  for (let i = 0; i < string.length; i++) {
    // check if char is open paren
    if (string[i] === "(") {
      counter += 1
    }
    if (string[i] === ")") {
