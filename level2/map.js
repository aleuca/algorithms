 ...
 * using the given function and
 * creates a new array from the
 * resultant items.
 * For example, if 'array' is
 * an array of numbers, then 'func'
 * is expected to take a single
 * parameter that is a number,
 * and returns a new item. The new
 * item will not always have the
 * same type as the original item.
 * Returns the new array.
 * The original array should
 * remain unchanged.
 *
 * Note: functions
 * like this (functions that take
 * other functions as parameters)
 * are sometimes referred to as
 * higher order functions.
 * This concept originates in
 * functional programming.
 */


function map(input, inputFunction) {
  let array = []
  for (element of input) {
    const result = inputFunction(element)
    array.push(result)
  }
  return array
}
